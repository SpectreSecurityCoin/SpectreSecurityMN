...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SPECTRESECURITYParams"
    else
        PARAMS_DIR="$HOME/.spectresecurity-params"
    fi
fi
...
Failed to fetch the SPECTRESECURITY zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SPECTRESECURITY - fetch-params.sh

This script will fetch the SPECTRESECURITY zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SPECTRESECURITY zkSNARK parameters. Note that it is
