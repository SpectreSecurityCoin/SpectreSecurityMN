...
class ReorgStakeTest(SpectresecurityTestFramework):
    ...
    def check_money_supply(self, expected_ssmn):
        # verify that nodes have the expected SSMN supply
        ssmn_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(ssmn_supply, [DecimalAmt(expected_ssmn)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that SSMN supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check SSMN supply...")
