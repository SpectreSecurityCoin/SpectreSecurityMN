...
"""Test spectresecurityd with different proxy configuration.

Test plan:
- Start spectresecurityd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on spectresecurityd side:
...
- Create spectresecurityds that connect to them
- Manipulate the spectresecurityds using addnode (onetry) an observe effects
...
class ProxyTest(SpectresecurityTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: spectresecurityd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
