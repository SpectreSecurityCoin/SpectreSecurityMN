...
The build script we'll be using is contained in the SPECTRESECURITY github repository ([contrib/gitian-build.py](https://github.com/spectresecurity-project/spectresecurity/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the SPECTRESECURITY repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/Spectresecurity/spectresecurity/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, spectresecurity-detached-sigs, and spectresecurity GitHub repos.
