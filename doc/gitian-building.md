...
*Setup instructions for a gitian build of SPECTRESECURITY Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SPECTRESECURITY
...
to the SPECTRESECURITY GitHub release page.
...
- [Building SPECTRESECURITY Core](#building-spectresecurity-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/spectresecurity-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/spectresecurity-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/spectresecurity-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/spectresecurity-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/spectresecurity-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/spectresecurity-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/spectresecurity-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@spectresecurity.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/spectresecurity-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/spectresecurity-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/spectresecurity-osx-*-build.assert
```
