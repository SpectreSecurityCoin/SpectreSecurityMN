...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/spectresecurity/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
SPECTRESECURITY - install-params.sh

This script will install the SPECTRESECURITY zkSNARK parameters and verify their
