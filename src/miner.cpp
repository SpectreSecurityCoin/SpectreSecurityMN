...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("SPECTRESECURITYMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("SPECTRESECURITYMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SPECTRESECURITYMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("spectresecurity-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SPECTRESECURITYMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("SPECTRESECURITYMiner exception");
    } catch (...) {
        LogPrintf("SPECTRESECURITYMiner exception");
    }

    LogPrintf("SPECTRESECURITYMiner exiting\n");
}
