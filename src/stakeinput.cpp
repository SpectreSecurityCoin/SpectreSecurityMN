...
CSsmnStake* CSsmnStake::NewSsmnStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CSsmnStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CSsmnStake(txPrev.vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CSsmnStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CSsmnStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CSsmnStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CSsmnStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CSsmnStake::GetUniqueness() const
{
    //The unique identifier for a SSMN stake is the outpoint
...
const CBlockIndex* CSsmnStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CSsmnStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CSsmnStake::ContextCheck(int nHeight, uint32_t nTime)
{
