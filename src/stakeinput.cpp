// Copyright (c) 2017-2022 The SPECTRESECURITY Core developers
...
CSsmnStake* CSsmnStake::NewSsmnStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CSsmnStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CSsmnStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}
...
CTxIn CSsmnStake::GetTxIn() const
{
