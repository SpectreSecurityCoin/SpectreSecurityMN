...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", SPECTRESECURITY_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", SPECTRESECURITY_PID_FILENAME));
#endif
...
#endif
    LogPrintf("SPECTRESECURITY version %s\n", version_string);
}
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SPECTRESECURITY_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if SPECTRESECURITY is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if SPECTRESECURITY is started while in a temporary directory.\n",
