...
void PrepareShutdown()
{
    ...
    util::ThreadRename("spectresecurity-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SPECTRESECURITY_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SPECTRESECURITY_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SPECTRESECURITY specific functionality (Masternodes, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), SPECTRESECURITY_MASTERNODE_CONF_FILENAME));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The SPECTRESECURITY Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("spectresecurity-loadblk");
...
/** Sanity checks
 *  Ensure that SPECTRESECURITY is running in a usable environment with all
...
#endif
    LogPrintf("SPECTRESECURITY version %s (%s)\n", version_string, CLIENT_DATE);
}

/** Initialize spectresecurity.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. SPECTRESECURITY Core is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single SPECTRESECURITY process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. SPECTRESECURITY Core is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //SPECTRESECURITY specific: zerocoin and spork DB's
                ...
                // SPECTRESECURITY: load previous sessions sporks if we have them.
