...
public:
    ...
    bool WriteZSSMNSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZSSMNSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZSSMNSeed_deprecated(uint256& seed);
    bool EraseZSSMNSeed();
    bool EraseZSSMNSeed_deprecated();

    bool WriteZSSMNCount(const uint32_t& nCount);
    bool ReadZSSMNCount(uint32_t& nCount);
