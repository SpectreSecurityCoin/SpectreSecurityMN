...
bool CLegacyZSsmnStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZSsmnStake from non zc-spend", __func__);
    ...
    *this = CLegacyZSsmnStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zssmn stake origin", __func__);

    // All good
    return true;
}

CLegacyZSsmnStake::CLegacyZSsmnStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
bool CLegacyZSsmnStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSSMN stake block: height %d outside range", __func__, nHeight);
