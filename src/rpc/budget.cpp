...
UniValue preparebudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"spectresecurity-address\" monthy-payment\n"
            ...
            "5. \"spectresecurity-address\":   (string, required) SPECTRESECURITY address to send payments to\n"
    ...
    if (!pwalletMain->CreateBudgetFeeTX(wtx, nHash, keyChange, false)) { // 50 SSMN collateral for proposal
...
UniValue submitbudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"spectresecurity-address\" monthly-payment \"fee-tx\"\n"
