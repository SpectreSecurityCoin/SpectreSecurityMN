...
const char * const SPECTRESECURITY_CONF_FILENAME = "spectresecurity.conf";
const char * const SPECTRESECURITY_PID_FILENAME = "spectresecurity.pid";
const char * const SPECTRESECURITY_MASTERNODE_CONF_FILENAME = "masternode.conf";


// SPECTRESECURITY only features
// Masternode
...
#else
    const char* pszModule = "spectresecurity";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SPECTRESECURITY
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SPECTRESECURITY
// Mac: ~/Library/Application Support/SPECTRESECURITY
// Unix: ~/.spectresecurity
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SPECTRESECURITY";
#else
    fs::path pathRet;
    ...
    return pathRet / "SPECTRESECURITY";
#else
    // Unix
    return pathRet / ".spectresecurity";
#endif
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SPECTRESECURITYParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SPECTRESECURITYParams
    // Mac: ~/Library/Application Support/SPECTRESECURITYParams
    // Unix: ~/.spectresecurity-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SPECTRESECURITYParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SPECTRESECURITYParams";
#else
    // Unix
    return pathRet / ".spectresecurity-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/spectresecurity/sapling-spend.params";
        sapling_output = "/usr/share/spectresecurity/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/spectresecurity/sapling-spend.params";
            sapling_output = "/usr/local/share/spectresecurity/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", SPECTRESECURITY_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", SPECTRESECURITY_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty spectresecurity.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override spectresecurity.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", SPECTRESECURITY_PID_FILENAME));
