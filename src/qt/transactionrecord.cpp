    ...
    for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (mine) {
            ...
            if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                // Received by SPECTRESECURITY Address
    ...
    for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (wtx.HasZerocoinMintOutputs())
                continue;
            // Sent to SPECTRESECURITY Address
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned SSMNs, op_return could be for a proposal/budget fee or another sort of data stored there.
...
bool IsZSSMNType(TransactionRecord::Type type)
{
    switch (type) {
        ...
        case TransactionRecord::ZerocoinSpend_Change_zSsmn:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZSSMN ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZSSMN);
}
