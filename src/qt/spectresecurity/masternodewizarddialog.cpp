...
#include "qt/spectresecurity/forms/ui_masternodewizarddialog.h"
    ...
    mnModel(_mnModel)
{
    ...
    ui->labelMessage1a->setText(formatHtmlContent(
                formatParagraph(tr("To create a SPECTRESECURITY Masternode you must dedicate %1 (the unit of SPECTRESECURITY) "
    ...
    if (walletModel->isRegTestNetwork()) {
        ui->lineEditPort->setText("7472");
    } else if (walletModel->isTestNetwork()) {
        ui->lineEditPort->setText("7372");
    } else {
        ui->lineEditPort->setText("7272");
    }
