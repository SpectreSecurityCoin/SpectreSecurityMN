...
#include "qt/spectresecurity/settings/forms/ui_settingsconsolewidget.h"
...
#include "qt/spectresecurity/settings/moc_settingsconsolewidget.cpp"
...
SettingsConsoleWidget::SettingsConsoleWidget(SPECTRESECURITYGUI* _window, QWidget *parent) :
...
void SettingsConsoleWidget::clear(bool clearHistory)
{
    ...
    messageInternal(CMD_REPLY, (tr("Welcome to the SPECTRESECURITY RPC console.") + "<br>" +
...
static bool PotentiallyDangerousCommand(const QString& cmd)
{
    ...
    if (cmd.size() >= 13 && cmd.leftRef(11) == "dumpprivkey") {
        // valid SPECTRESECURITY Transparent Address
        std::vector<std::string> args;
    ...
    if (cmd.size() >= 18 && cmd.leftRef(16) == "exportsaplingkey") {
        // valid SPECTRESECURITY Shield Address
        std::vector<std::string> args;
        parseCommandLineSettings(args, cmd.toStdString());
