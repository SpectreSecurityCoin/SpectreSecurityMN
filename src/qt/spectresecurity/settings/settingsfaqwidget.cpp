...
#include "qt/spectresecurity/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(SPECTRESECURITYGUI* parent, MNModel* mnModel) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableSSMN,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableSSMN,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableSSMN,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("SPECTRESECURITY is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("SPECTRESECURITY utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableSSMNContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received SPECTRESECURITY requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your SPECTRESECURITY wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableSSMN->setText(unspendableSSMNContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of SPECTRESECURITY with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("spectresecurity-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.spectresecurity.io'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(mnModel->getMNCollateralRequiredAmount(), BitcoinUnits::SSMN)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in SPECTRESECURITY Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the SPECTRESECURITY network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableSSMN, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableSSMN);});
    ...
    if (parent)
        connect(parent, &SPECTRESECURITYGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
