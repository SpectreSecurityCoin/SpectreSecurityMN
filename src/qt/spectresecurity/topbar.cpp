...
#include "qt/spectresecurity/topbar.h"
#include "qt/spectresecurity/forms/ui_topbar.h"
#include "qt/spectresecurity/lockunlock.h"
#include "qt/spectresecurity/qtutils.h"
#include "qt/spectresecurity/receivedialog.h"
...
TopBar::TopBar(SPECTRESECURITYGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSsmn, ui->labelAmountTopShieldedSsmn}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSsmn}, "amount-topbar");
    setCssProperty({ui->labelPendingSsmn, ui->labelImmatureSsmn}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SPECTRESECURITY")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // SSMN Total
    QString totalSsmn = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // SSMN
    // Top
    ui->labelAmountTopSsmn->setText(totalTransparent);
    ui->labelAmountTopShieldedSsmn->setText(totalShielded);
    // Expanded
    ui->labelAmountSsmn->setText(totalSsmn);
    ui->labelPendingSsmn->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureSsmn->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
