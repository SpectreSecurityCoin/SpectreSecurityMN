...
#include "qt/spectresecurity/forms/ui_snackbar.h"
#include "qt/spectresecurity/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SPECTRESECURITYGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SPECTRESECURITYGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
