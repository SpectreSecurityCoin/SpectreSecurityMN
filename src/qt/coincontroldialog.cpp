...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select SSMN Outputs to Spend" :
            "Select Shielded SSMN to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::SSMN;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show SPECTRESECURITY address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
