...
#define URI_SCHEME "spectresecurity"
...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZssmn)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZssmn)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZssmn);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZssmn)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZssmn)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZssmn);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter SPECTRESECURITY address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no SPECTRESECURITY: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::SSMN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert spectresecurity:// to spectresecurity:
    //
    //    Cannot handle this later, because spectresecurity:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SSMN, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", SPECTRESECURITY_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SPECTRESECURITY (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SPECTRESECURITY (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "SPECTRESECURITY.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SPECTRESECURITY*.lnk
...
{
    return GetAutostartDir() / "spectresecurity.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a spectresecurity.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=SPECTRESECURITY (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=SPECTRESECURITY (regtest)\n";
        else
            optionFile << "Name=SPECTRESECURITY\n";
