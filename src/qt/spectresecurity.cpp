...
{
    return QCoreApplication::translate("spectresecurity-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in spectresecurity.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in spectresecurity.qrc)
...
/** Class encapsulating SPECTRESECURITY Core startup and shutdown.
...
/** Main SPECTRESECURITY application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (SPECTRESECURITYGUI)
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SPECTRESECURITYGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // SPECTRESECURITY: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SPECTRESECURITYGUI::handlePaymentRequest);
        connect(window, &SPECTRESECURITYGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(spectresecurity_locale);
    Q_INIT_RESOURCE(spectresecurity);
