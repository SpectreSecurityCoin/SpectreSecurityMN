...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SSMN);
    ...
    if (settings.contains("nAnonymizeSpectresecurityAmount") || reset)
        gArgs.SoftSetArg("-anonymizespectresecurityamount", settings.value("nAnonymizeSpectresecurityAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in spectresecurity.cpp during shotdown to also remove the window positions
